...
def batchnorm_backward(dout, cache):
    ...
    dx = dx_norm/np.sqrt(x_var+eps)+dx_var*2*(x-x_mean)/N+dx_mean/N+dx_var*(-2)*np.sum(x-x_mean,axis=0)/N/N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for i in range(0,(H-HH+1),stride):
        for j in range(0,(W-WW+1),stride):
            for n in range(0,N):
                out[n,:,(i//stride),(j//stride)] =np.sum(w*x_pad[n,:,i:i+HH,j:j+WW],axis=(1,2,3))+b 
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(0,(H-HH+1),stride):
        for j in range(0,(W-WW+1),stride):
            for n in range(0,N):
                out[n,:,(i//stride),(j//stride)] =np.max(x[n,:,i:i+HH,j:j+WW],axis=(1,2))
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(0,(H-HH+1),stride):
        for j in range(0,(W-WW+1),stride):
            for n in range(0,N):
                for c in range(0,C):
                    ind = np.unravel_index(np.argmax(x[n,c,i:i+HH,j:j+WW], axis=None), x[n,c,i:i+HH,j:j+WW].shape)
                    temp = np.zeros_like(x[n,c,i:i+HH,j:j+WW])
                    temp[ind] = 1
                    dx[n,c,i:i+HH,j:j+WW]+=dout[n,c,(i//stride),(j//stride)]*temp
